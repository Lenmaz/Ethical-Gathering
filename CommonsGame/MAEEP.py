...
def ethical_equilibrium_computation():
    ...
    for agent in range(number_of_agents):
        try:
            ...
            np.save("ref_policy" + str(agent) + "_C" + str(COMMON_POOL_LIMIT) + ".npy", policy)
...
def multi_agent_ethical_embedding_process():
    ...
    for agent in range(number_of_agents):
        np.save("ref_policy" + str(agent) + "_C" + str(COMMON_POOL_LIMIT) + ".npy", ethical_eq[agent])

    ethical_weight = solution_weight_vector_computation()

    print("The solution weight vector for a common pool of capacity C = ", COMMON_POOL_LIMIT, " is : ", ethical_weight)

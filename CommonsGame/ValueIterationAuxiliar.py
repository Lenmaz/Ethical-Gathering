...
def probsV_on_agent_apples(mode, agent, action, Q_ag, V, original_state, state_prima, tabularRL, action_space, forced_pool_apples, apples_in_ground, forcing_next_apples=False):
    ...
    elif 0 < checks_agent_before < TOO_MANY_APPLES:  # If agent was in state s = few apples for itself
        ...
        if checks_agent_before != checks_agent_after:
            ...
            if checks_agent_before < checks_agent_after:  # +1 in apples
                ...
                if action == TAKE_DONATION and mode == "lex":
                    ...
                    new_original = new_state(agent, original_state, True)
                    new_prima = new_state(agent, state_prima, True, forced_agent_apples=TOO_MANY_APPLES - 1)
                    best_next_action = np.argmax(scalarised_Qs(len(action_space), Q_ag[new_prima][action_space], [0.0, 1.0]))
    ...
    elif checks_agent_before >= TOO_MANY_APPLES:
        if action == TAKE_DONATION:
            if checks_agent_before < checks_agent_after:
                new_prima = new_state(agent, state_prima, True)
...
def create_model():
    ...
    model = np.zeros((len_state_space, environment.action_space.n, environment.action_space.n, 21 + 4))
                    ...
                    for c_state in common_pool_states:
                        ...
                        for action in action_space:
                            ...
                            for action2 in action_space:
                                ...
                                if nObservations[0][1] == 64 and not ap_state[0]:
                                    #print(ap_state, nObservations)
                                    nObservations[0][1] = 32

                                if nObservations[0][2] == 64 and not ap_state[1]:
                                    #print(ap_state, nObservations)
                                    nObservations[0][2] = 32

                                if nObservations[0][5] == 64 and not ap_state[2]:
                                    #print(ap_state, nObservations)
                                    nObservations[0][5] = 32


                                for i in range(21):
                                    model[state][action][action2][i] = nObservations[0][i]

                                for i in range(21, 23):
                                    model[state][action][action2][i] = nRewards[0][i-21]

                                for i in range(23, 25):
                                    model[state][action][action2][i] = nRewards[1][i-23]

...
def Optimistic_Linear_Support(agent, epsilon):
    ...
    try:
        v_ethical = np.load(v_folder+"V_"+str(agent)+"_C"+str(COMMON_POOL_LIMIT)+"_"+str(3000.0)+".npy")
    except:
        v_ethical = SolveSOMDP(discount_factor=0.8, weight_vector=[1.0, 3000.0], algorithm="VI", learner=agent, mode="scalarisation")
        np.save(v_folder+"V_" + str(agent) + "_C" + str(COMMON_POOL_LIMIT) + "_" + str(3000.0) + ".npy", v_ethical)
    print("Ethical policy computed.")

    try:
        v_unethical = np.load(v_folder+"V_"+str(agent)+"_C"+str(COMMON_POOL_LIMIT)+"_"+str(0.0)+".npy")
    except:
        v_unethical = SolveSOMDP(discount_factor=0.8, weight_vector=[1.0, 0.0], algorithm="VI", learner=agent, mode="scalarisation")
        np.save(v_folder+"V_" + str(agent) + "_C" + str(COMMON_POOL_LIMIT) + "_" + str(0.0) + ".npy", v_unethical)
    ...
    while new_ethical_weight > epsilon:
        ...
        try:
            v_unethical = np.load(v_folder+"V_" + str(agent) + "_C" + str(COMMON_POOL_LIMIT) + "_" + str(ethical_weight) + ".npy")
        except:
            ...
            np.save(v_folder+"V_" + str(agent) + "_C" + str(COMMON_POOL_LIMIT) + "_" + str(ethical_weight) + ".npy", v_unethical)
